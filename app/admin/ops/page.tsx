...
export default function AdminOpsPage() {
  ...
  const loadChecks = useCallback(async () => {
    try {
      ...
      (payload.data || []).forEach((c: OpsCheck) => {
      ...
      <div className="max-w-5xl mx-auto space-y-12">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-2">
            <Badge variant="verified" className="px-4 py-1 text-xs uppercase font-black tracking-widest">System Readiness</Badge>
            <SectionHeading 
          ...
          <div className={`p-6 rounded-[2rem] border-2 flex items-center gap-6 transition-all ${
            ...
            <div className={`h-12 w-12 rounded-2xl flex items-center justify-center shadow-sm ${
              ...
              {isGo ? <Rocket className="h-6 w-6" /> : <AlertTriangle className="h-6 w-6" />}
            </div>
            <div>
              <p className={`text-[10px] font-black uppercase tracking-widest ${isGo ? "text-emerald-600" : "text-rose-600"}`}>
                Current Gate Status
              </p>
              <p className={`text-2xl font-black uppercase tracking-tightest ${isGo ? "text-emerald-900" : "text-rose-900"}`}>
