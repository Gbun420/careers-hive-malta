...
import type { JobReport, ReportStatus } from "@/lib/trust/schema";
  ...
  if (error?.error?.code === "SUPABASE_NOT_CONFIGURED") {
    return (
      <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
        Connect Supabase to review reports. {" "}
      ...
      {items.map((item) => (
        ...
        >
          ...
          <div className="mt-4 space-y-2">
            <Label htmlFor={`resolution-${item.id}`}>Resolution notes</Label>
            <Textarea
              ...
              rows={3}
              placeholder="Optional notes for the report resolution."
            />
          </div>
        </div>
      ))}
